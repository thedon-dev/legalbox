    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        ...
        <div className="space-y-6">
          {/* Document Info */}
          <div>
            <h3 className="text-sm font-semibold text-muted-foreground mb-3">Document Information</h3>
          ...
          {/* Hash */}
          <div>
            <h3 className="text-sm font-semibold text-muted-foreground mb-3">Document Hash (BlockDAG)</h3>
            <div className="flex items-center gap-2 bg-secondary p-3 rounded-lg">
              <code className="text-xs font-mono text-foreground flex-1 break-all">{document.hash}</code>
              <Button variant="ghost" size="icon" onClick={handleCopyHash}>
                {copied ? <Check className="w-4 h-4 text-primary" /> : <Copy className="w-4 h-4" />}
              </Button>
            </div>
            <p className="text-xs text-muted-foreground mt-2">This hash proves ownership on the BlockDAG ledger</p>
          </div>

          {/* Sharing Info */}
          <div>
            <h3 className="text-sm font-semibold text-muted-foreground mb-3">Sharing</h3>
            <Card className="p-4 bg-secondary border-border">
              <p className="text-sm text-foreground">
                Shared with <span className="font-bold text-primary">{document.sharedWith}</span> recipient
          ...
          {/* Permissions */}
          <div>
            <h3 className="text-sm font-semibold text-muted-foreground mb-3">Your Permissions</h3>
