...
const createShare = asyncHandler(async (req, res) => {
  ...
  if (
    req.user &&
    req.user.walletAddress !== doc.ownerWallet &&
    req.user.roleType !== "organization"
  )
  ...
  await Log.create({
    docId,
    actionType: "share",
    performedByWallet: req.user?.walletAddress,
    details: { linkId },
  });
  ...
  if (req.user?.email)
    sendNotification({
      ...
      text: `Your document ${doc.name} was shared.`,
    }).catch(() => {});
...
const accessShare = asyncHandler(async (req, res) => {
  ...
  if (link.accessCode && link.accessCode !== code)
...
const revokeShare = asyncHandler(async (req, res) => {
  ...
  if (req.user?.email)
    sendNotification({
      to: req.user.email,
      subject: "Share revoked",
      text: `Share link for ${doc.name} was revoked.`,
    }).catch(() => {});
  res.json({ link });
});
...
module.exports = { createShare, accessShare, revokeShare, getSharedWithMe };
